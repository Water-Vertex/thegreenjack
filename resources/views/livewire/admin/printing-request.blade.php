<div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Printing Requests</h1>
                <p class="text-gray-600 mt-1">Manage and track all customer printing service requests</p>
            </div>
            <div class="text-sm text-gray-500">
                Total Requests: <span class="font-semibold text-gray-900">{{ $totalRequests }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end">
                @if($search || $perPage != 10 || $sortField != 'created_at' || $sortDirection != 'desc')
                <button wire:click="clearFilters"
                    class="w-full px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200 transition">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->phone ?? 'N/A' }}</td>
                        <td class="px-6 py-4">
                            <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                                {{ Str::limit($request->service, 30) }}
                            </span>
                        </td>
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-12 text-center">
                            ...
                            <p class="text-gray-500">No printing requests found</p>
                            @if($search)
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-900">Printing Request #{{ $viewingRequest->id }}</h3>
                ...
                <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900 mt-1">{{ $viewingRequest->name }}</p>
                            </div>
                            <div>
                                <span class="text-sm text-gray-500">Email Address:</span>
                                <p class="text-sm text-gray-900 mt-1">
                                    <a href="mailto:{{ $viewingRequest->email }}" class="text-blue-600 hover:underline">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">
                                    <a href="tel:{{ $viewingRequest->phone }}" class="text-blue-600 hover:underline">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">{{ $viewingRequest->created_at->format('F d, Y h:i A') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    <select wire:change="updateStatus({{ $viewingRequest->id }}, $event.target.value)"
                                        class="text-sm px-3 py-1 rounded-lg border border-gray-300 focus:ring-[#5C9F01] focus:border-[#5C9F01]">
                                        <option value="pending" {{ $viewingRequest->status == 'pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="processing" {{ $viewingRequest->status == 'processing' ? 'selected' : '' }}>Processing</option>
                                        <option value="completed" {{ $viewingRequest->status == 'completed' ? 'selected' : '' }}>Completed</option>
                                        <option value="cancelled" {{ $viewingRequest->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <p class="text-sm text-gray-700 whitespace-pre-wrap">{{ $viewingRequest->message }}</p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">#{{ $viewingRequest->id }}</p>
                            </div>
                            <div>
                                <span class="text-sm text-gray-500">Last Updated:</span>
                                <p class="text-sm text-gray-900 mt-1">{{ $viewingRequest->updated_at->format('F d, Y h:i A') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
                    ...
                    <button onclick="if(confirm('Are you sure you want to delete this request?')) { @this.deleteRequest({{ $viewingRequest->id }}) }"
